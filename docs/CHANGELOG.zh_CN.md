...
- 电池满充显示校正：CW2017 跑开源 BSP 的通用 Li-Poly 曲线时，满电点高于
  实际电芯的充电终止电压，满充读数常停在 99%。采集任务现同时读取电压，
  UI 侧经主机测试的 `ui_pixel_battery_display_soc()` 校正：soc>=99 且
  电压>=4150 mV 按 100% 显示，其余读数原样呈现（含读不到时的灰块）。
  快照仍携带原始读数，校正只作用于显示。
- 新增社区发布助手要求的只读串口截屏协议 FAP_SCREENSHOT_V1：独立任务
  监听 USB 串口命令行，收到 `FAP_SCREENSHOT_V1` 后用 LVGL 快照把当前屏
  幕渲染为紧排 RGB565 帧并回传（快照写入编译期静态预留的整屏缓冲——运行
  时堆碎片化拿不出 150KB 连续块；二进制窗口内暂停日志输出）。应用不启动
  REPL、固件里原本无人安装串口驱动，该任务自行安装 USB 串口驱动并把
  控制台切到驱动通道（对齐官方 REPL 组合）。该命令仅观察屏幕，不重启、
  不刷写、不触碰任何设置与凭据。
- 上屏读数平滑：模型层新增非对称 EMA（快起约 130 ms、慢落约 0.5 s，
  Q8 定点、首帧直通），大数字、音量条与分区颜色改用平滑值，显示不再
  每 80 ms 跳变；峰值/均值统计与告警状态机有意保留原始值，瞬态捕捉
  与告警响应不受影响。会话清零（OK 短按）保留平滑器状态，读数不再
  闪一下 0.0。攻击/释放/收敛行为纳入主机测试。
- 音量检测页 UI review 修复：28 号大读数改为固定宽度右对齐，>= 100 dB
  时不再与 dB 单位、状态徽章相撞；"90" 刻度改为右对齐贴面板内缘
  （此前左对齐会溢出）；阈值行右侧新增分区文字（QUIET..EXTREME），
  并跟随麦克风异常状态刷新；修复一个潜在 bug——告警边沿标志在样式
  更新前被覆盖，吉祥物此前永远不会切换到告警反白样式。
