...
- **FAP_SCREENSHOT_V1 第二次落地：流式采集与 USB TX 环**（Y2Lin）— 拦截刷新回调的流式采集管线、无 PSRAM 下的有界环形槽内存预算、TX 环默认 256B 的坑、机器可读失败原因码，以及 Web Serial 主机侧四条纪律。见 [`../experiences/y2lin/screenshot-streaming-and-usb-tx-ring.zh_CN.md`](../experiences/y2lin/screenshot-streaming-and-usb-tx-ring.zh_CN.md)。
