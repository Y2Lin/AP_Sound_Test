...
- **FAP_SCREENSHOT_V1, Second Landing: Streamed Capture and the USB TX Ring** (Y2Lin) — a flush-callback streaming capture pipeline, bounded ring slots for no-PSRAM memory budgets, the 256B default TX-ring trap, machine-readable failure codes, and four Web Serial host-side disciplines. See [`../experiences/y2lin/screenshot-streaming-and-usb-tx-ring.md`](../experiences/y2lin/screenshot-streaming-and-usb-tx-ring.md).
